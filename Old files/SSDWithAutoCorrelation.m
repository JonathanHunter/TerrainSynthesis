function SSDWithAutoCorrelation()
    image(generateImage());
end
...
function image = generateImage()
    ...
    for r = 1:dim(1)
        for c = 1:dim(2)
            image(r,c) = sumOfSquaresDifference(image1, image2, r,c, sumI1);
        end
    end
    image
end

function ssd = sumOfSquaresDifference(image1, image2, r, c, sumI1)
    dim1 = size(image1);
    dim2 = size(image2);
    ...
    sumI2 = sum(temp(:).^2);
    corr = xcorr2(image1, temp);
    ssd = sumI1 + sumI2 - 2 * (corr(dim1(1),dim1(2)));
